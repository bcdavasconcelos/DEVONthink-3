...
tell application id "DNtp"
	...
	if theMainText contains "<!-- " then
		...
		repeat with theFile in theFiles
			...
			set theSearchPattern to my replaceText(theSearchPattern, "|", "\\|") -- Prepare to look for the commented mark using regex
			set theSearchPattern to theSearchPattern & "(.|\\s)+?\\*\\*\\*\\" & linefeed & "</details>" -- Search pattern including transclusion mark and transcluded text
	...
	if theFiles is not {} then
		repeat with theFile in theFiles
			...
			if theSearch is not {} then
				...
				set theSubText to regex change theSubText search pattern "(aliases: .+)" & linefeed replace template ""
				set theSubText to regex change theSubText search pattern "(m: .+)" & linefeed replace template ""
				set theSubText to regex change theSubText search pattern "(tags: .+)" & linefeed replace template ""
			--	set theSubText to regex change theSubText search pattern "(#{" & n & ".+}" & linefeed replace template ""
				...
				--				set theSign to "\\* \\* \\*"
				--				set theSign to ""
				set theSearchName to "[" & theSearchName & "]"
				--				set theSign to "```" & linefeed & theSearchName & linefeed & "```" & linefeed
				set theReplacement to "<!-- " & theFile & " -->  " & linefeed & "<details>
  <summary>" & theSearchName & "</summary>" & theSubText & "  " & linefeed & theSubURL & linefeed & "***" & linefeed & "</details>"
	...
	if theFiles is not {} then
		repeat with theFile in theFiles
			...
			if theSearch is not {} then
				...
				-- end randomize
				set theSearchName to "[" & theSearchName & "]"
				set theReplacement to "<!-- " & theFile & " -->" & linefeed & "<details>" & "<summary>" & theSearchName & "</summary>" & linefeed & theSubText & "  " & theSubURL & linefeed & "</details>"
				set theMainText to my replaceText(theMainText, theFile, theReplacement)
